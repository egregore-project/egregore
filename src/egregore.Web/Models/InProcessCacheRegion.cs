...
namespace egregore.Web.Models
{
    internal sealed class InProcessCacheRegion<TRegion> : ICacheRegion<TRegion>
    {
        ...
        {
            return _cache.TryGetValue(GetCacheKey(key), out value);
        }

        public TValue Get<TValue>(in string key)
        {
            return _cache.Get<TValue>(GetCacheKey(key));
        }

        public void Set<TValue>(in string key, TValue value, in TimeSpan ttl)
        {
            _cache.Set(GetCacheKey(key), value, TimeZoneLookup.Now.Timestamp.Add(ttl));
        }

        public void Set<TValue>(in string key, TValue value)
        {
        ...
        {
            return $"{typeof(TRegion).Name}:{key}";
        }
    }
}